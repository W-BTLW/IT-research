...
1. CPU를 최대한 놀리지 않고 활용하기
2. 평균 대기 시간은 최소로 줄이기
3. 처리량(=단위 시간당 처리하는 일의 양)은 최대로 늘리기
...
① FCFS — "번호표 순서대로"
편의점 계산대처럼 그냥 온 순서대로 처리. 근데 앞사람이 카드 결제 오류나서 10분 붙잡고 있으면? 뒷사람들 다 손해봄 (이게 콘보이 효과). 한번 시작하면 중간에 못 끊음(비선점).
...
### SJF (Short - Job - First)
② SJF — "빨리 끝날 사람부터"
계산대에서 "물건 1개인 사람 먼저 오세요" 하는 거. 전체적으로 보면 대기 줄이 제일 빨리 줄어드는 방법(수학적으로 증명된 최적). 근데 카트 가득 찬 사람은 계속 밀려서 못 계산하게 될 수도 있음(기아 현상).
...
③ SRTF — "SJF의 실시간 버전"
계산 중인데 갑자기 물건 1개 든 사람이 뒤에 서면, 하던 계산 멈추고 그 사람부터 처리해주는 것. SJF보다 더 예민하게(=선점형으로) 반응함.
...
④ Priority — "VIP 먼저"
그냥 "이 사람 먼저 해주세요" 우선순위표가 있는 것. 근데 계속 VIP만 오면 일반 손님은 하염없이 기다림. 그래서 Aging이라는 걸 씀 — 오래 기다린 사람은 시간이 지날수록 VIP 등급으로 승격시켜주는 거예요.
...
⑤ Round Robin — "1인당 3분씩만"
모두에게 딱 3분씩만 주고, 시간 다 되면 아직 안 끝났어도 뒤로 가서 다시 줄 서게 하는 방식. 공평하긴 한데, 이 3분이 너무 길면 사실상 FCFS랑 똑같아지고, 너무 짧으면 왔다갔다(문맥 전환) 하느라 시간을 낭비하게 됨.
