...
### 3. 전통적 제어 방식의 한계 : 왜 분산 전략이 필수인가?
일부 개발자들은 <mark>@Transactional</mark>이나 <mark>synchronized</mark> 키워드를 통해 트래픽이나 동시성을 제어할 수 있다고 생각한다.
<br>
하지만 이 도구들은 <mark>단일 서버의 내부 질서를 유지하는 도구</mark>일 뿐, 대규모 분산 환경의 복잡한 문제를 해결하기에는 근본적인 한계가 있다.

<br>

#### 3.1 @Transactional의 한계 : 트래픽 통제 불가
@Transactional은 데이터베이스의 데이터 일관성을 보장하는데 사용된다.
<br>
즉, 여러 쿼리를 하나의 작업 묶음으로 처리하는 것이 주 목적이다.
- 트래픽 제어 수단이 아님 : 트랜잭션은 외부에서 들어오는 요청의 개수나 속도를 제어하는 기능(Rate Limiting)이 없음. 요청이 한꺼번에 몰리면 트랜잭션은 모든 요청이 안전하게 DB에 도달하도록 하려 할 뿐, 요청 자체를 거부하거나 지연시키지 않는다.
- 중앙 집중형 ID 생성의 병목 : DB의 Auto-Increment를 사용하며 트랜잭션을 적용하면, 모든 서버가 ID를 얻기 위해 중앙의 단일 DB 테이블에 락(Lock)을 걸게 된다. 이는 곧 트래픽 처리 능력을 떨어뜨리는 심각한 병목 현상을 초래한다.

<br>

#### 3.2 synchronized의 한계 : 분산 환경에서의 무력화
synchronized 키워드는 단일 JVM(Java Virtual Machine) 내의 스레드 간 동시성을 제어하는 데 사용된다.
<br>
특정 코드 영역을 한 번에 하나의 스레드만 실행하도록 강제한다.
- 분산 환경에서는 무용지물 : 최신 대규모 서비스는 여러 대의 서버(다중 JVM)에 분산되어 실행된다. synchronized는 서로 다른 서버(JVM)간의 동시성을 제어할 수 없다.
- 충돌 발생 : 서버A와 서버B에서 동시에 ID를 생성하거나 동일한 리소스를 수정할 때, 각 서버의 synchronized는 상대 서버의 동작을 알지 못하므로 데이터 충돌을 막을 수 없다. 따라서 분산 락(Distributed Lock)과 같은 별도의 분산 제어 기술이 필요하다.
